...
namespace sprout {
	namespace detail {
		...
		struct search_one_fwd {
			...
			SPROUT_CONSTEXPR
			search_one_fwd(ForwardIterator1 last1, ForwardIterator2 last2, BinaryPredicate pred, ForwardIterator1 first1_)
				: last1(last1), last2(last2), pred(pred), first1_(first1_) {}

			SPROUT_CONSTEXPR type
			search_one_impl_1(type const& current, difference_type n) {
				return sprout::tuples::get<2>(current) ? current
					...
					: sprout::detail::search_one_impl_1(
						sprout::detail::search_one_impl_1(current, n / 2),
						n - n / 2)
					;
			}

			SPROUT_CONSTEXPR type
			search_one_impl(type const& current, difference_type n) {
				return sprout::tuples::get<2>(current) ? current
					: sprout::tuples::get<1>(current) == last2 ? type(first1_, last2, true)
					: sprout::tuples::get<0>(current) == last1 ? type(last1, last2, true)
					: sprout::detail::search_one_impl(
						sprout::detail::search_one_impl_1(current, n),
						n * 2)
					;
			}
		};
		...
		{
			typedef sprout::tuples::tuple<ForwardIterator1, ForwardIterator2, bool> type;
			return sprout::tuples::get<0>(sprout::detail::search_one_fwd(last1, last2, pred, first1).search(type(first1, first2, false), 1));
		}
